...
from model.modeling_genmc_team import GenMC
import json
import argparse
...
def get_input_feature(samples, max_source_length, max_len_gen, choice_num, external_sent_num=None):
    ...
    answers = torch.tensor(answers, dtype=torch.long).view(-1,).to(device)
...
def eval(model, test_examples, tokenizer, eval_batch_size, choice_num, max_len, max_len_gen, external_sent_num):
    ...
    for step in step_trange:
        ...
        scores = torch.argmax(scores, 1).view(-1, choice_num)
        scores = scores.cpu().detach().tolist()
        answers = answers.view(-1, choice_num).cpu().detach().tolist()
        p_anss = []
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_path_train",
                        default='./data/qasc/in_hourse/train.jsonl',
                        required=True,
                        type=str)
    parser.add_argument("--data_path_dev",
                        default='./data/qasc/in_hourse/dev.jsonl',
                        required=True,
                        type=str)
    parser.add_argument("--data_path_test",
                        default='./data/qasc/in_hourse/test.jsonl',
                        required=True,
                        type=str)
    ...
    output_model_path = args.output_dir + "/" + args.name_save_prix + "/" + file_name + "/"
    path_save_result =  args.output_dir + args.name_save_prix + "/results/" + file_name + "/"
